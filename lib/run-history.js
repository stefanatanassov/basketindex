...
function buildRunRecord(job) {
  ...
  if (job.config.adapterId === 'lidl' && job.stats.pagesScanned >= job.config.endPage) {
    warnings.push({
      ...
      message: `Configured page range ended at page ${job.config.endPage}. Additional receipt pages may exist beyond this window.`
    });
  }
  ...
  if (failedCount > 0) {
    warnings.push({
      ...
      message: `${failedCount} receipt(s) failed to extract and were not included.`
    });
  }
