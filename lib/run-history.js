...
function buildRunRecord(job) {
  ...
  const followupMeta = job.followupMeta || null;

  // For follow-ups with 0 new receipts: mark as info status, not done
  const effectiveStatus = followupMeta && !followupMeta.hasNewReceipts ? 'info' : (job.status === 'completed' ? 'done' : (job.status === 'error' ? 'error' : 'partial'));
