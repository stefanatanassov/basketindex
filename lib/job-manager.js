...
import { parseReceipt } from '../adapters/lidl/receipt-parser.js';
...
async function doApiProcess(job) {
  ...
  // Lidl: cookie-based API — no token check needed
  if (adapter && adapter.id === 'lidl') {
    try {
      ...
      const rawReceipt = parseReceipt(doc, task.receipt_id);
      rawReceipt.source_url = `https://www.lidl.bg/mre/purchase-detail?t=${task.receipt_id}`;
