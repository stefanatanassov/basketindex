    ...
    try {
      ...
      while (page <= endPage) {
        const result = await Promise.race([
          sendToTab(job.listingTabId, { type: 'LIDL_FETCH_LISTING', page }),
          new Promise(r => setTimeout(() => r({ success: false, error: 'Listing relay timed out' }), 15000))
        ]);
  ...
  if (adapter && adapter.id === 'lidl') {
    try {
      ...
      const rawReceipt = await Promise.race([
        sendToTab(job.listingTabId, {
          type: 'LIDL_PARSE_RECEIPT_HTML',
          html: '__FETCH__', // signal to fetch from API
          receiptId: task.receipt_id
        }),
        new Promise(r => setTimeout(() => r({ success: false, error: 'Detail relay timed out' }), 25000))
      ]);
