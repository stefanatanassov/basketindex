...
import { readTokenFromPage, isTokenExpired, getAccountIdFromToken } from '../adapters/metro/auth.js';
...
async function doApiDiscover(job) {
  ...
  const accountId = getAccountIdFromToken(token) || job.config.metroAccountId || '751453c0-5946-11e9-9e80-c70dee944b33';
  ...
  job._metroToken = token;
  job._metroAccountId = accountId;
...
async function doApiProcess(job) {
  ...
  try {
    ...
    job.inFlight = job.inFlight.filter(id => id !== task.receipt_id);
    failReceipt(job, task.receipt_id, err.message, job.config.maxRetries);
    await saveJob(job);
  }
}
