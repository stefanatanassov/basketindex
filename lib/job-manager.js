...
import { fetchAllListings, fetchDetail, fetchListing } from '../adapters/lidl/api-client.js';
import { addRun, buildRunRecord, loadRuns } from './run-history.js';

let jobRunTimer = null;

async function relayToTab(job, tabId, message, timeoutMs = 15000) {
  // Retry up to 3 times — content script injection may lag behind page load.
  for (let attempt = 0; attempt < 3; attempt++) {
    const result = await Promise.race([
      sendToTab(tabId, message),
      new Promise(r => setTimeout(() => r({ success: false, error: 'Relay timed out' }), timeoutMs))
    ]);
    ...
    if (attempt < 2) {
      ...
      // Re-verify tab exists
      try { await chrome.tabs.get(tabId); } catch (_) {
        return { success: false, error: 'Relay tab closed' };
      }
    }
  }

  return { success: false, error: 'Relay failed after retries' };
}
...
async function doApiDiscover(job) {
  ...
  // Lidl: cookie-based API via content-script relay.
  // SW cannot include page cookies in fetch, so we route API calls
  // through the content script on an open Lidl tab.
  if (adapter && adapter.id === 'lidl') {
    const startPage = job.config.startPage || 1;
    const endPage = job.config.endPage || 9999;

    // Open a Lidl tab for content-script relay
    if (!job.listingTabId) {
      const lidlTab = await createTab('https://www.lidl.bg/mre/purchase-history?page=1', TAB_TYPES.listing);
      job.listingTabId = lidlTab.id;
      job.workers.push(createWorkerEntry(lidlTab.id, TAB_TYPES.listing));
      await saveJob(job);
      await waitForTabLoad(lidlTab.id, 15000);
    ...
    try {
      ...
      while (page <= endPage) {
        const result = await relayToTab(job, job.listingTabId, { type: 'LIDL_FETCH_LISTING', page });

        if (!result || !result.success || !result.data) {
          throw new Error(result?.error || 'Listing relay failed');
        }
        ...
        for (const item of items) {
          job.queue.push({
            receipt_id: item.id,
            url: `https://www.lidl.bg/mre/api/v1/tickets/${item.id}?country=BG&languageCode=bg-BG`,
            lidlListingItem: item,
            attempts: 0, lastError: null,
            enqueuedAt: new Date().toISOString()
          });
        }

        job.stats.totalPages = Math.ceil(totalCount / 10);
        job.stats.pagesScanned = page;
      ...
      console.warn('Lidl API listing error:', err.message);
      job.status = 'error';
      job.phase = 'done';
      job._lastError = err.message || 'Relay failed';
      await saveJob(job);
      return;
    }
  ...
  // Metro: token-based API — need auth tab + token acquisition
  ...
  if (!authResult || !authResult.success || !authResult.token) {
    job.status = 'error';
    job.phase = 'done';
    job._lastError = 'Metro token acquisition failed';
    await saveJob(job);
    return;
  }
  ...
  if (isTokenExpired(token)) {
    job.status = 'error';
    job.phase = 'done';
    job._lastError = 'Metro session expired';
    await saveJob(job);
    return;
  }
  ...
  try {
    ...
    console.warn('Metro API listing error:', err.message);
    clearApiState(job.jobId);
    job.status = 'error';
    job.phase = 'done';
    job._lastError = err.message || 'Metro API error';
    await saveJob(job);
    return;
  }
...
async function doApiProcess(job) {
  ...
  // Lidl: content-script relay for detail fetch + HTML parse.
  // Both API call (needs cookies) and DOMParser (needs DOM context)
  // are handled by the content script on the open Lidl tab.
  if (adapter && adapter.id === 'lidl') {
    try {
      job.inFlight.push(task.receipt_id);
      await saveJob(job);

      // Fetch detail + parse via content-script relay
      const rawReceipt = await relayToTab(job, job.listingTabId, {
        type: 'LIDL_PARSE_RECEIPT_HTML',
        html: '__FETCH__',
        receiptId: task.receipt_id
      }, 25000);

      if (!rawReceipt || !rawReceipt.success) {
        throw new Error(rawReceipt?.error || 'Detail relay failed');
      }
      ...
      if (adapter) {
        ...
        if (normResult.success) {
          ...
          console.warn('Lidl normalization warning:', normResult.error);
      ...
      console.warn('Lidl API detail error:', err.message);
      job.inFlight = job.inFlight.filter(id => id !== task.receipt_id);
      failReceipt(job, task.receipt_id, 'Lidl API error: ' + err.message, job.config.maxRetries);
      await saveJob(job);
    }
    return;
  }

  // Metro: token-based API
  ...
  try {
    ...
    console.warn('Metro API detail error:', err.message);
...
async function finalizeJob(job) {
  ...
  // Deduplication for follow-up runs
  if (job.config.derivedFromRunId) {
    const allRuns = await loadRuns();

    // Build set of known receipt IDs from parent chain
    const knownIds = new Set();
    const parentId = job.config.derivedFromRunId;
    const parentRun = allRuns.find(r => r.runId === parentId);
    if (parentRun) {
      for (const rc of (parentRun.results || [])) {
        const rid = rc.receipt?.id || '';
        if (rid) knownIds.add(rid);
      }
    }
    // Also check follow-ups of the same parent
    const siblings = allRuns.filter(r => r.derivedFromRunId === parentId);
    for (const sib of siblings) {
      for (const rc of (sib.results || [])) {
        const rid = rc.receipt?.id || '';
        if (rid) knownIds.add(rid);
      }
    }

    const fetchedCount = job.completed.length;
    const deduped = job.completed.filter(c => {
      if (!c._normalized) return false;
      const rid = c._normalized.receipt?.id || '';
      return rid && !knownIds.has(rid);
    });
    const duplicateCount = fetchedCount - deduped.length;

    job.completed = deduped;
    job.followupMeta = {
      fetchedReceiptCount: fetchedCount,
      duplicateReceiptCount: duplicateCount,
      addedReceiptCount: deduped.length,
      hasNewReceipts: deduped.length > 0
    };
  }

  // Persist as immutable run record in history
  try {
    const run = buildRunRecord(job);
    await addRun(run);
  } catch (err) {
    console.error('Failed to persist run history:', err);
  }
