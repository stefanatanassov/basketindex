...
async function doApiDiscover(job) {
  ...
  // Lidl: cookie-based API — no token acquisition needed. Call API directly.
  // Lidl API always returns 10 items per page (size param is ignored).
  if (adapter && adapter.id === 'lidl') {
    ...
    try {
      ...
      while (page <= endPage) {
        ...
        if (items.length < 10) break; // partial page = last page
        page++;
      }
    } catch (err) {
...
async function doApiProcess(job) {
  ...
  // Lidl: cookie-based API fetch + content-script relay for HTML parsing.
  // DOMParser is unavailable in SW context, so we send htmlPrintedReceipt
  // to an open Lidl tab's content script which has DOM APIs.
  if (adapter && adapter.id === 'lidl') {
    try {
      job.inFlight.push(task.receipt_id);
      await saveJob(job);

      const detailData = await fetchDetail(task.receipt_id);
      const htmlStr = detailData.ticket?.htmlPrintedReceipt || '';

      // Ensure Lidl listing tab exists for content-script relay
      let lidlTabId = job.listingTabId;
      if (!lidlTabId) {
      ...
      const rawReceipt = await Promise.race([
        ...
        new Promise(r => setTimeout(() => r({ success: false, error: 'HTML parse relay timed out' }), 20000))
      ]);

      if (!rawReceipt || !rawReceipt.success) {
        throw new Error(rawReceipt?.error || 'HTML parse relay failed');
      }
