...
function collectItems(runs, scopeRunId, selectedIds) {
  const collected = [];
  ...
  for (const run of runs) {
    ...
    for (const receipt of (run.results || [])) {
      ...
      for (const it of (receipt.items || [])) {
        ...
        if (price <= 0) continue;
        collected.push({ date, bucket: getQuarterKey(date), name, displayId: did, runId: run.runId, unitPrice: price, quantity: it.quantity?.value || 1, retailer: run.retailer });
      }
    }
  }
  return collected;
...
function buildAggregateSeries(runs, scopeRunId, aggregation) {
  const fn = aggregation === 'year' ? getYearKey : aggregation === 'month' ? getMonthKey : getQuarterKey;
  const items = collectItems(runs, scopeRunId, null);
  ...
  return [{ name: 'ALL', label: 'Всички артикули', totalObservations: points.reduce((s, p) => s + p.observations, 0), bucketCount: points.length, points }];
}

function buildSelectedSeries(runs, scopeRunId, selectedIds, aggregation) {
  const fn = aggregation === 'year' ? getYearKey : aggregation === 'month' ? getMonthKey : getQuarterKey;
  const items = collectItems(runs, scopeRunId, selectedIds);
  const seriesMap = new Map();
  for (const it of items) {
    if (!seriesMap.has(it.displayId)) seriesMap.set(it.displayId, { name: it.displayId, label: `${retailerLabel(it.retailer)} · ${it.name}`, buckets: new Map(), totalObservations: 0 });
  ...
  for (const s of seriesMap.values()) {
    ...
    series.push({ name: s.name, label: s.label, totalObservations: s.totalObservations, bucketCount: s.buckets.size, points });
  }
...
export { loadRuns, getRunOptions, getItemOptions, buildAggregateSeries, buildSelectedSeries, convertToPercentage, convertToIndex, getAllBuckets, getTrendSummary, getQuarterLabel };
