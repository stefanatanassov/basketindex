...
function getTopProductId(runs, scopeRunId) {
...
function getTrendSummary(series, mode) {
  ...
  const obsNote = lastPt.observations < 5 ? ` (само ${lastPt.observations} покупки)` : '';
  return `Средна платена цена в последния период: <strong>${last.toFixed(2)} лв</strong>.<span class="ts-formal">${obsNote}</span>`;
}
