...
function buildSelectedSeries(runs, scopeRunId, selectedIds, aggregation, dateFrom, dateTo) {
  ...
  for (const it of items) {
    if (!seriesMap.has(it.displayId)) seriesMap.set(it.displayId, { name: it.displayId, label: `${retailerLabel(it.retailer)} · ${it.name}`, runId: it.runId, retailer: it.retailer, buckets: new Map(), totalObservations: 0, totalEur: 0, totalBgn: 0 });
    ...
    for (const receipt of (run.results || [])) {
      ...
      for (const it of (receipt.items || [])) {
        ...
        rows.push({
          ...
          productName: name,
          productLabel: `${retailerLabel(run.retailer)} · ${name}`,
...
export { loadRuns, getRunOptions, getItemOptions, getAvailableDateRange, buildAggregateSeries, buildSelectedSeries, collectEvidence, convertToPercentage, convertToIndex, getAllBuckets, getTrendSummary, getQuarterLabel };
