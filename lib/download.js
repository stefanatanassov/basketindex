...
function receiptsToCsvRows(receipts) {
  ...
  for (const receipt of receipts) {
    ...
    const primary = (currency.primary || '').toUpperCase();
    const secondary = (currency.secondary || '').toUpperCase();
    ...
    for (const item of items) {
      ...
      let unitEur = '', lineEur = '', unitBgn = '', lineBgn = '';

      if (primary === 'EUR') {
        unitEur = pricing.unit_price_primary || '';
        lineEur = pricing.line_total_primary || '';
      }
      if (secondary === 'EUR') {
        unitEur = pricing.unit_price_secondary || unitEur;
        lineEur = pricing.line_total_secondary || lineEur;
      }
      if (primary === 'BGN') {
        unitBgn = pricing.unit_price_primary || '';
        lineBgn = pricing.line_total_primary || '';
      }
      if (secondary === 'BGN') {
        unitBgn = pricing.unit_price_secondary || unitBgn;
        lineBgn = pricing.line_total_secondary || lineBgn;
      }
