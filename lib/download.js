...
const CSV_COLUMNS = [
  ...
  'item_discount_eur', 'item_discount_bgn',
  'item_discount_count', 'item_discount_descriptions', 'item_discount_promotion_ids',
  'tax_type'
];
...
function receiptsToCsvRows(receipts) {
  ...
  for (const receipt of receipts) {
    ...
    for (const item of items) {
      ...
      const discEur = itemDiscounts.reduce((s, d) => s + (d.amount_primary || 0), 0);
      const discBgn = itemDiscounts.reduce((s, d) => s + (d.amount_secondary || 0), 0);
      const discDesc = itemDiscounts.map(d => d.description || '').filter(Boolean).join('; ');
      const discPromos = itemDiscounts.map(d => d.promotion_id || '').filter(Boolean).join('; ');
      ...
      rows.push([
        ...
        unitEur, lineEur,
        unitBgn, lineBgn,
        discEur > 0 ? discEur.toFixed(2) : '',
        discBgn > 0 ? discBgn.toFixed(2) : '',
        itemDiscounts.length > 0 ? itemDiscounts.length : '',
        discDesc,
        discPromos,
        tax.type || ''
